...
import { upVoteThread, downVoteThread, neutralizeVoteThread } from '../states/threads/action';
import { asyncFetchThreadsAndUsers } from '../states/threads/action';
...
function Homepage() {
  ...
  const { threads, users } = useSelector((state) => state.threads);
  ...
  const upVoteMutation = useMutation((threadId) => dispatch(upVoteThread(threadId)), {
    onSuccess: () => queryClient.invalidateQueries('threadsAndUsers'),
  });

  const downVoteMutation = useMutation((threadId) => dispatch(downVoteThread(threadId)), {
    onSuccess: () => queryClient.invalidateQueries('threadsAndUsers'),
  });

  const neutralizeVoteMutation = useMutation((threadId) => dispatch(neutralizeVoteThread(threadId)), {
    onSuccess: () => queryClient.invalidateQueries('threadsAndUsers'),
  });
  ...
  const voteHandler = (threadId, action) => {
    const previousVotes = localVotes[threadId] !== undefined
      ? localVotes[threadId]
      : threads.find((thread) => thread.id === threadId).votes;
    let newVotes;
